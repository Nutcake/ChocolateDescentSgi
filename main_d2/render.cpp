...
void render_side(segment* segp, int sidenum)
{
	...
	if (sidep->type == SIDE_IS_QUAD) {
		...
		// -- Old, slow way --	//	Regardless of whether this side is comprised of a single mQuad, or two triangles, we need to know one normal, so
...
#if 0		//this stuff could probably just be deleted //[ISB] k
#endif
...
int build_segment_list(int start_seg_num, int window_num)
{
	int	lcnt, scnt, ecnt;
	int	l, c;
	int	ch;
